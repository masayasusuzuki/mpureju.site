...
export async function generateMetadata({
  ...
  if (!treatment) return {};
  return {
    title: `${treatment.title}｜目・糸・リフトの整形`,
...
export default async function LiftTreatmentPage({
  ...
  // サイドバー用: 同じピラーの他の施術
  const allEye = await getTreatmentsByPillar("lift");
  const otherTreatments = allEye.contents.filter((t) => t.slug !== slug);
        ...
        <div className="absolute inset-0 flex flex-col justify-end pb-10 md:pb-12">
          <div style={{ paddingLeft: "max(1.5rem, calc((100vw - 1200px) / 2 + 2rem))" }}>
            <nav className="flex items-center gap-2 text-xs text-[var(--color-brand-dark)]/50 mb-4 tracking-wider">
              ...
              <span>/</span>
              <Link href="/lift" className="hover:text-[var(--color-brand-dark)] transition-colors">糸・リフト</Link>
              <span>/</span>
