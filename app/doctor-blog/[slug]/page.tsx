...
export async function generateStaticParams() {
  const data = await getBlogList({ filters: "slug[contains]doctor", limit: 100 });
...
export async function generateMetadata({
  ...
  if (!post) return {};
  return {
    title: `${post.title}｜院長ブログ｜Maison PUREJU`,
    description: `Maison PUREJU院長ブログ「${post.title}」`,
  };
}

export default async function DoctorBlogDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const [post, allData, campaignData] = await Promise.all([
    getBlogBySlug(slug),
    getBlogList({ filters: "slug[contains]doctor", limit: 100 }),
    getCampaigns(),
  ]);
  if (!post) notFound();
  ...
  return (
    <ArticleDetailLayout
      ...
      sidebarExtra={
        <SidebarList title="最新記事" items={latestPosts} basePath="/doctor-blog" />
      }
      backLink={{ href: "/doctor-blog", label: "院長ブログ一覧に戻る" }}
    />
  );
}
