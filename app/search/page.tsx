...
import { searchTreatments, searchColumns, searchCases } from "@/lib/microcms/client";
...
export default async function SearchPage({
  ...
  const [treatmentResults, caseResults, columnResults, priceResults] = keyword
    ? await Promise.all([
        searchTreatments(keyword),
        searchCases(keyword, 3),
        searchColumns(keyword, 3),
        searchPriceItems(keyword),
        // ローディングアニメーションを最低2秒表示させるためのタイマー
        new Promise<void>((r) => setTimeout(r, 2000)),
      ] as const).then(([t, ca, c, p]) => [t, ca, c, p] as const)
  ...
  return (
    <div className="py-16 md:py-24">
      ...
      <div className="section-container">
        ...
        {keyword && (
          <div className="search-results mt-12 space-y-16">
            ...
            {caseResults && (
              <section>
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-1 h-6 bg-[var(--color-brand-gold)]" />
                  <h2 className="font-serif text-xl text-[var(--color-brand-dark)] tracking-wide">症例実績</h2>
                  <span className="text-xs text-[var(--color-brand-gold)] border border-[var(--color-brand-gold)]/40 px-2 py-0.5">{caseResults.totalCount}件</span>
                  {caseResults.totalCount > 3 && (
                    <Link
                      href={`/case?q=${encodeURIComponent(keyword)}`}
                      className="ml-auto text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-brand-gold)] transition-colors shrink-0"
                    >
                      もっと見る →
                    </Link>
                  )}
                </div>
                {caseResults.contents.length > 0 ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {caseResults.contents.map((c) => (
                      <Link
                        key={c.id}
                        href={`/case/${c.slug}`}
                        className="group bg-white border border-[var(--color-brand-brown)]/10 overflow-hidden hover:border-[var(--color-brand-gold)]/30 transition-colors"
                      >
                        ...
                        <div className="p-4">
                          ...
                          <p className="text-sm text-[var(--color-brand-dark)] leading-snug group-hover:text-[var(--color-brand-gold)] transition-colors line-clamp-2">
                            {c.treatment_label}
                          </p>
                          {c.timing && (
                            <p className="text-[10px] text-[var(--color-text-secondary)] mt-1">{c.timing}</p>
                          )}
                        </div>
                      </Link>
                    ))}
                  ...
                  <p className="text-sm text-[var(--color-text-secondary)] py-4">該当する症例が見つかりませんでした</p>
                )}
              </section>
            )}
            ...
            {columnResults && (
              <section>
                <div className="flex items-center gap-4 mb-8">
                  ...
                  {columnResults.totalCount > 3 && (
                    <Link
                      href={`/column?q=${encodeURIComponent(keyword)}`}
                      className="ml-auto text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-brand-gold)] transition-colors shrink-0"
                    >
                      もっと見る →
                    </Link>
