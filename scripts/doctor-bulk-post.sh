...
# public/doctor/ 配下の全ディレクトリを順番に投稿する
...
find public/doctor -maxdepth 1 -type d -name '[0-9]*' | sort | while IFS= read -r FULL_PATH; do
  ...
  OUTPUT=$(node scripts/microcms-doctor-blog-post.js "$FULL_PATH" 2>&1)
