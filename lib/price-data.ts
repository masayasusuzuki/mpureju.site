/**
 * 料金データ（共有モジュール）
 * /price/ ページと施術詳細ページの両方から参照する。
 * TODO: Phase 2 で microCMS `prices` API に移行
...
export const HIFUKA_TABS: SubTab[] = [
  {
    label: "ヒアルロン酸",
    rows: [
...
export const TENTEKI_ROWS: PriceRow[] = [
...
export const NAIFUKU_ROWS: PriceRow[] = [
...
export const KESHOUHIN_TABS: SubTab[] = [
  {
    label: "レカルカ",
    rows: [
...
export const SONOTA_TABS: SubTab[] = [
  {
    label: "診察料",
    rows: [
...
/**
 * 施術名で料金データを検索する。
 * category が完全一致 or 部分一致する行と、その直後の option 行（category が空文字）をまとめて返す。
 */
export function findPriceRowsByTitle(title: string): PriceRow[] {
  const allTabs = [...HIFUKA_TABS, ...GEKA_TABS, ...KESHOUHIN_TABS, ...SONOTA_TABS];
  const allRows = [
    ...allTabs.flatMap((tab) => tab.rows),
    ...TENTEKI_ROWS,
    ...NAIFUKU_ROWS,
  ];
