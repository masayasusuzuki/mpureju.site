...
export const FAQ_CATEGORIES: FaqPageCategory[] = [
  {
    id: "mouth",
    label: "口元の施術について",
    en: "Mouth",
...
/** キーワードでFAQページのQ&Aを検索 */
export function searchFaqPage(keyword: string): FaqSearchResult[] {
  ...
  for (const cat of FAQ_CATEGORIES) {
    for (let i = 0; i < cat.items.length; i++) {
      ...
      if (item.q.toLowerCase().includes(kw) || item.a.toLowerCase().includes(kw)) {
        results.push({ catId: cat.id, itemIndex: i, q: item.q, a: item.a });
      }
    }
  }
